...
class Queue {
    ...
    dequeue() {
        return this.processes.splice(0, 1);
    }
    ...
    isEmpty() {
        return this.processes.length < 1 ? true : false;
    }
    ...
    manageTimeSlice(currentProcess, time) {
        ...
        if (this.quantumClock > this.quantum) {
            //move to a lower queue
            this.quantumClock = 0;
            let process = this.dequeue(); //process == currentProcess
            if (!process.isFinished()) {
                handleInterrupt(queue, process, SchedulerInterrupt.LOWER_PRIORITY);
            }
        }
    }
    ...
    doCPUWork(time) {
        let process = this.processes.peek();
    ...
    emitInterrupt(source, interrupt) {
        ...
        let process = this.processes.splice(index, 1);
        switch (interrupt) {
            case SchedulerInterrupt.PROCESS_BLOCKED:
                handleInterrupt(this.processes, process, SchedulerInterrupt.PROCESS_BLOCKED);
                break;
            case SchedulerInterrupt.PROCESS_READY:
                handleInterrupt(this.processes, process, SchedulerInterrupt.PROCESS_READY);
                break;
            default:
                break;
        }
